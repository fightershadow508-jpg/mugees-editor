...
function adminPanel(){
 ...
 return `<div class="admin-banner"><span class="kicker" style="color:#a7f3ff">Admin Control Center</span><h1 style="margin:8px 0 6px">Mughees Edtior Platform</h1><p>Manage students, classes, programs, approved earnings, payout requests and notifications.</p></div><div class="admin-stats"><div class="dash-card"><small>Total Students</small><strong>${state.students.length}</strong></div><div class="dash-card"><small>Active Subscriptions</small><strong>${activeSubs}</strong></div><div class="dash-card"><small>Total Revenue</small><strong>${money(totalRevenue)}</strong></div><div class="dash-card"><small>Pending Withdrawals</small><strong>${pending}</strong></div></div><div class="dashboard-grid"><div class="panel"><div class="panel-head"><h3>Students</h3><button class="btn small light" data-admin="students">Manage</button></div><div class="list">${state.students.map(s=>`<div class="list-item"><div class="meta"><strong>${esc(s.name)}</strong><small>${esc(s.id)} · ${esc(s.program)}</small></div><span class="amount green">${money(s.available)}</span></div>`).join('')}</div></div><div class="panel"><div class="panel-head"><h3>Upcoming classes</h3><button class="btn small light" data-admin="live">Manage</button></div><div class="list">${state.classes.filter(c=>c.status==='Upcoming').map(c=>`<div class="list-item"><div class="meta"><strong>${esc(c.title)}</strong><small>${niceDate(c.date)} · ${esc(c.time)}</small></div></div>`).join('')}</div></div></div>`
}
function adminStudents(){return `<div class="subpage-head"><div><h1>Manage Users</h1><p style="color:#748195">User management and access control.</p></div></div><div class="panel"><div class="table-wrap"><table class="table"><thead><tr><th>Profile</th><th>Name</th><th>Join Date</th><th>Status</th><th>Actions</th></tr></thead><tbody>${state.students.map(s=>{const storedAvatar=(s.id===state.session?.studentId)?localStorage.getItem('userAvatar'):null;const fallbackUrl=\`https://ui-avatars.com/api/?name=\${encodeURIComponent(s.name)}&background=random&color=fff&rounded=true\`;const avatarImg=storedAvatar||s.avatar||fallbackUrl;const isSuspended=s.status==='Suspended';return \`<tr><td><img src="\${avatarImg}" style="width:36px;height:36px;border-radius:50%;object-fit:cover;border:2px solid rgba(255,255,255,0.1);"></td><td><strong>\${esc(s.name)}</strong><br><small style="color:#8fa1b8">\${esc(s.id)}</small></td><td>\${niceDate(s.joinDate)}</td><td><span class="tag \${isSuspended?'red':'green'}">\${esc(s.status||'Active Creator')}</span></td><td><button class="btn small primary" data-action="edit-user" data-id="\${s.id}">Edit</button> <button class="btn small orange" data-action="suspend-user" data-id="\${s.id}">\${isSuspended?'Activate':'Suspend'}</button> <button class="btn small danger" data-action="delete-user" data-id="\${s.id}">Delete</button></td></tr>\`;}).join('')}</tbody></table></div></div>`}
